...
const compareTwoArraysOfObjects = (arr1: object[], arr2: object[]) => {
  return (
    ...
    arr1.every((arr1Obj) =>
      arr2.some((arr2Obj) =>
        Object.keys(arr1Obj).every((key) => arr1Obj[key] === arr2Obj[key])
      )
    )
  );
};

export const taxonomyItemType = defineType({
  name: "taxonomyItem",
  title: "Taxonomy item",
  type: "object",
  icon: Waypoints,
            ...
            filter: async ({ document, parent, getClient }) => {
              ...
              function getTaxonomyItems(obj) {
                for (var property in obj) {
                  if (property == "_type" && obj[property] == "taxonomyItem") {
                    return documentItems.push(obj);
                  }
                  if (Array.isArray(obj[property])) {
                    for (let item in obj[property]) {
                      getTaxonomyItems(obj[property][item]);
                    }
                  }
                  if (
              ...
              const parentAttribute = documentItems.filter((item) => {
                if (!item.terms) return false;
                return compareTwoArraysOfObjects(item.terms, parent);
              })[0].taxonomyAttribute;
